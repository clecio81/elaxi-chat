...
const Mensagem = require("./messageRouter.js");
...
class OperatorConnectionHandler extends ChatConnectionHandler {
  ...
  attachHandlers () {
    ...
    this.socket.on(AppConstants.EVENT_SEND_FILE, (message) => {
      console.log('solicitação:', message);
      this._sendFile(message);
    });
      ...
      .then(customer => {
        ...
        if (customer.mode === CustomerStore.MODE_AGENT) {
          return Promise.reject(
            new CustomerModeError('Este usuário não precisa de suporte humano no momento.')
          );
        }
...
_sendFile (message) {
    ...
    this.router.customerStore
      ...
      .then(customer => {
        // Check if we're in agent or human mode
        // If in agent mode, ignore the input
        console.log('Got customer file: ', JSON.stringify(customer));
        if (customer.mode === CustomerStore.MODE_AGENT) {
          return Promise.reject(
            new CustomerModeError('Este usuário não precisa de suporte humano no momento.')
          );
        ...
        return this.router._relayOperatorMessage(message)
          ...
          .then(() => {
            const customerConnection = this.router.customerConnections[customerId];
           return customerConnection._sendFile(utterance);
          });
      })
      .catch(error => {
        console.log('Error handling operator input: ', error);
...
_sendedFile () {
    console.log('Sending file buffy to operator:');
    this.socket.emit(AppConstants.EVENT_SEND_FILE);
