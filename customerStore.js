...
class CustomerStore {
  ...
  getOrCreateCustomer (customerId) {
    if (!customerId || customerId.length === 0) {
      return Promise.reject(new Error('Você deve especificar um ID de cliente'));
    }
  ...
  setCustomer (customerId, customerData) {
    ...
    if (!customerId || customerId.length === 0 || !customerData) {
      return Promise.reject(new Error('Você deve especificar um ID de cliente e fornecer dados para armazenar'));
    }
