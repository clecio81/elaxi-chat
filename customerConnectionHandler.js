...
class CustomerConnectionHandler extends ChatConnectionHandler {
  ...
  attachHandlers () {
    ...
    this.socket.on(AppConstants.EVENT_SENDED_FILE, (message) => {
      console.log('agr sim');
      this._sendedFile(message);
    });
  }
  ...
  _sendFile () {
    console.log('Sending file to customer:');
    this.socket.emit(AppConstants.EVENT_SEND_FILE);
    return Promise.resolve();
  }
  
_sendedFile (utterance) {
	console.log("router")
	 this.router.customerStore
      .getOrCreateCustomer(this.socket.id)
      .then(customer => {
        // Tell the router to perform any next steps
        return this.router._sendedFile(utterance, customer, this.socket.id);
      });
     }
  ...
  _sendErrorToCustomer () {
    ...
    this.socket.emit(AppConstants.EVENT_SYSTEM_ERROR, {
      type: 'Error',
      message: 'Temos um problema aqui, o problema sou eu. Não é você.'
    });
  }
}
