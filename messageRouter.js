...
class MessageRouter {
  ...
  _handleCustomerConnection (socket) {// Copyright 2017, Google, Inc.
// Licensed under the Apache License, Version 2.0 (the 'License');
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
...
class MessageRouter {
  ...
  _sendUtteranceToOperator (utterance, customer, isAgentResponse) {
    console.log('Sending msg for operator');
  ...
  _sendedFile (utterance, customer, isAgentResponse) {
    console.log('Sended img to any operators 2');
    if (Array.isArray(utterance)) {
      utterance.forEach(message => {
 this.operatorRoom.emit(AppConstants.EVENT_CUSTOMER_MESSAGE,
    this._operatorMessageObject(customer.id, message, isAgentResponse));
...
_sendFile (utterance, customer, isAgentResponse) {
    console.log('Sending file');
    if (Array.isArray(utterance)) {
      utterance.forEach(message => {
        this.operatorRoom.emit(AppConstants.EVENT_SEND_FILE,
          this._operatorMessageObject(customer.id, message, isAgentResponse));
      });
    } else { 
      this.operatorRoom.emit(AppConstants.EVENT_SEND_FILE,
...
if(str > "17:30:30" || str.includes("AM")){
	return this.customerStore
      ...
      .then(() => {
        ...
        const output = [ response.queryResult.fulfillmentText, "Atualmente não temos nenhum agente disponível, pois estamos fora do nosso horário comercial. Entre em contato via email:<br><a href='mailto:elaxigerency@gmail.com'>elaxigerency@gmail.com</a><br><h5>Horário de funcionamento</h5><h6>13:30:00 PM até as 17:30:30 PM.</h6><br><img  style='width:170px' src='https://firebasestorage.googleapis.com/v0/b/forty-1a729.appspot.com/o/assets%2F5%20Sem%20T%C3%ADtulo_20220511211357.png?alt=media&token=33abdfeb-29e2-4021-8f4d-bae7612a8037' alt='Elaxi logo'>" 
        ];
        // Also send everything to the operator so they can see how the agent responded
        this._sendUtteranceToOperator(output, customer, false);
        return output;
      });
}else{
